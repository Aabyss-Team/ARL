...
- **在 `CentOS7 and 8` 和 `Ubuntu20.04` 系统上，直接拉取运行本项目内的 `misc/setup-arl.sh` 即可安装，源码和docker安装都集成在了脚本中解放双手，docker安装我自己的docker镜像在shell脚本内修复了需要进入docker输入 `rabbitmqctl` 的bug，集成了[@本间白猫](https://github.com/honmashironeko) 师傅的docker镜像**
